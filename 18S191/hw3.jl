...
unused_letters = let
	[c for c in "abcdefghijklmnopqrstuvwxyz "][sample_freqs .== 0]
end
...
double_letters = [c for c in "abcdefghijklmnopqrstuvwxyz " if sample_freq_matrix[index_of_letter(c), index_of_letter(c)] != 0]
...
most_likely_to_follow_w = "abcdefghijklmnopqrstuvwxyz "[argmax(sample_freq_matrix[index_of_letter('w'), :])]
...
most_likely_to_precede_w = "abcdefghijklmnopqrstuvwxyz "[argmax(sample_freq_matrix[:, index_of_letter('w')])]
...
row_col_answer = md"""
Sum of rows: $(sum(sample_freq_matrix, dims=1))

Sum of columns: $(sum(sample_freq_matrix, dims=2))

The interpretation of these is the distribution of each character.
"""
...
function ngrams(words, n)
	all_ngrams = Vector{Vector{Any}}()
	for i in 1:length(words)-n+1
		ngram = Vector()
		for j in 1:n
			push!(ngram, words[i+j-1])
		end
