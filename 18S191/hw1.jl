...
student = (name = "Whistle bee", kerberos_id = "jazz")
...
function create_bar()
	...
	vector = zeros(100)
	center_index = len ÷ 2
	vector[center_index-10:center_index+10] .= 1
	return vector
end
...
function vecvec_to_matrix(vecvec)
	nrows = length(vecvec)
	ncols = length(vecvec[1])
	mat = zeros(eltype(vecvec[1]), nrows, ncols)
...
function matrix_to_vecvec(matrix)
	[matrix[i, :] for i = 1:size(matrix, 1)]
end
...
function mean_colors(image)
	red = map((pixel) -> pixel.r, image)
	green = map((pixel) -> pixel.g, image)
	blue = map((pixel) -> pixel.b, image)
...
begin
	...
	function quantize(color::AbstractRGB)
		...
		return typeof(color)(quantize(color.r), quantize(color.g), quantize(color.b))
	end
...
function invert(color::AbstractRGB)
	typeof(color)(1 - color.r, 1 - color.g, 1 - color.b)
end
...
begin
	...
	function noisify(x::Number, s)
		myclamp(x + rand() * 2 * s - s)
	end
	
	function noisify(color::AbstractRGB, s)
		snoisify(c) = noisify(c, s)
		typeof(color)(snoisify(color.r),
			          snoisify(color.g),
			          snoisify(color.b))
	end
...
# ╔═╡ 77d82da0-ef47-11ea-2fbe-9729eeaf44bd
typeof(philip)
...
function extend(v, i)
	function myclamp(x::Number, from::Number, to::Number)
		min(max(x, from), to)
	end
	v[myclamp(i, 1, length(v))]
end
...
function blur_1D(v, l)
	function window(i)
		[extend(v, j) for j = i-l:i+l]
	end
	[mean(window(i)) for i = 1:length(v)]
end
...
# ╔═╡ 5381c43a-ef4b-11ea-0f86-e9afa4bbb5da
...
function convolve_vector(v, k)
	...
	[sum(window(i) .* k) for i = 1:length(v)]
end
...
function gaussian_kernel(n)
	...
	prenorm = [G(i) for i = -l:l]
	return prenorm /= sum(prenorm)
end
...
@bind gaussian_kernel_size_1D Slider(0:8, show_value=true)
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	mat_rows, mat_cols = size(M)
	nrows, ncols = size(K)
	l1 = (nrows-1) ÷ 2
	l2 = (ncols-1) ÷ 2
	R = similar(M)
	for i = 1:mat_rows, j = 1:mat_cols
		tmp = similar(M, size(K))
		for ti = 1:nrows, tj = 1:ncols
			tmp[ti, tj] = extend_mat(M, i+ti-l1-1, j+tj-l2-1)
		end
		R[i, j] = sum(tmp .* K)
	end
	return R
...
function with_sobel_edge_detect(image)
	channel = Float64.(Gray24.(image))
	Gx = convolve_image(channel, [
		+1 0 -1
		+2 0 -2
		+1 0 -1
	])
	Gy = convolve_image(channel, [
		+1 +2 +1
	     0  0  0
		-1 -2 -1
	])
	Gtotal = sqrt.(Gx.^2 + Gy.^2)
	return Gtotal
end
