...
Initially it was doing 3 allocations in the main loop. Two for the two slices plus one for the vcat.

As there was 342 rows * 3 allocations = 972

Afterwards, we only do 2 allocations = 684
"""
...
We avoided basically all allocations. This was because after `img′` is allocated, all copying is done directly to `img′` instead of having another copy step in between. The other two are likely the `size` and `length` calls which return arrays.
"""
...
function greedy_seam(energies, starting_index::Int)
	rows, cols = size(energies)
	indices = zeros(Int, rows)
	indices[1] = starting_index
	for c in 2:rows
		min_ind = max(starting_index-1, 1)
		max_ind = min(starting_index+1, cols)
		diff = Int(starting_index == 1)
		starting_index = starting_index - 2 + diff + argmin(energies[c, min_ind:max_ind])
		indices[c] = starting_index
	end
	return indices
end
...
function least_energy(energies, i, j)
	...
	energies′ = [
		...
		for j′ = j-1:j+1 if j′ != 0 && j′ != cols+1
	]
	j′ = argmin(energies′)
	return energies′[j′] + energies[i, j], j - 2 + diff + j′
end
...
Essentially it does a breadth first search from the bottom up.
...
function memoized_least_energy(energies, i, j, memory)
	...
	diff = Int(j == 1)
	energies′ = [
		memoized_least_energy(energies, i+1, j′, memory)[1]
		for j′ = j-1:j+1 if j′ != 0 && j′ != n+1
	]
	j′ = argmin(energies′)
	calc_energy = energies′[j′] + energies[i, j]
	next_index = j - 2 + diff + j′
	memory[(i, j)] = calc_energy
	return calc_energy
end
...
function recursive_memoized_seam(energies, starting_pixel)
	...
	j = starting_pixel
	_ = memoized_least_energy(energies, 1, j, memory)
	
	for i=2:m
		next_index = j - 2 + Int(j == 1) + argmin([
			memory[i, j′]
			for j′ = j-1:j+1 if j′ != 0 && j′ != n+1
		])
		seam[i] = next_index
...
function matrix_memoized_least_energy(energies, i, j, memory)
	...
	diff = Int(j == 1)
	energies′ = [
		matrix_memoized_least_energy(energies, i+1, j′, memory)[1]
		for j′ = j-1:j+1 if j′ != 0 && j′ != n+1
	]
	j′ = argmin(energies′)
	calc_energy = energies′[j′] + energies[i, j]
...
function matrix_memoized_seam(energies, starting_pixel)
	...
	seam[1] = starting_pixel
	j = starting_pixel
	_ = matrix_memoized_least_energy(energies, 1, j, memory)
...
function least_energy_matrix(energies)
	...
	for i = m-1:-1:1
		for j = 1:n
			least_energies[i, j] += minimum([
				least_energies[i+1, j′]
				for j′ = j-1:j+1 if j′ != 0 && j′ != n+1
			])
		end
	end
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	...
	for i=2:m
		@inbounds next_index = j - 2 + Int(j == 1) + argmin([
			least_energies[i, j′]
			for j′ = j-1:j+1 if j′ != 0 && j′ != n+1
		])
		@inbounds seam[i] = next_index
	end
...
pika = decimate(load(download("https://art.pixilart.com/901d53bcda6b27b.png")), 80)
...
# ╔═╡ 1b558658-173e-11eb-21e1-156a15ac4b9d
shrink_n(pika, 5, recursive_memoized_seam)
