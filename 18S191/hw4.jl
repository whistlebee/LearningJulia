...
function frequencies(values)
	freq_dict = Dict()
	for v in values
		if haskey(freq_dict, v)
			freq_dict[v] += 1 / length(values)
		else
			freq_dict[v] = 1 / length(values)
		end
	end
...
# ╔═╡ b6f93f5e-1776-11eb-07c8-0526ba6cd71b
@bind N_interactive Slider(1:100000, show_value=true)

# ╔═╡ bad1018e-1776-11eb-2b90-934b78a54d58
let
...
let 
	y = 0.001:0.01:1
	mean = (x) -> sum(x) / length(x)
	x = [mean(do_experiment(p, 10000)) for p in y]
	plot(x, y)
end
...
# ╔═╡ c220e7c8-1779-11eb-2430-6f551521d588
Integer(test_status)
...
# ╔═╡ b404b14c-177c-11eb-3efd-256602b21b94
function bump_num_infected!(agent::Agent)
...
function sir_mean_plot(simulations::Vector{<:NamedTuple})
	...
	function make_mean(simulations, aspect)
		features = map(aspect, simulations)
		mean_aspect = zeros(Float64, T)
		mean = (x) -> sum(x) / length(x)
		for t = 1:T
			mean_aspect[t] = mean([features[s][t] for s = 1:length(simulations)])
		end
		return mean_aspect
	end

	p = plot()
	S_mean = make_mean(simulations, (sim) -> sim.S)
	I_mean = make_mean(simulations, (sim) -> sim.I)
	R_mean = make_mean(simulations, (sim) -> sim.R)
...
# ╔═╡ c5e74a7a-18f1-11eb-3155-ab2985cc5806
@bind p_infection Slider(0:0.01:0.3)

# ╔═╡ cfa2ea42-18f1-11eb-3324-9b7fd1f8ed15
@bind p_recovery Slider(0:0.005:0.1)
...
# ╔═╡ f75c6ddc-18f2-11eb-05b9-6775a7f97587
function sir_mean_error_plot(simulations::Vector{<:NamedTuple})
	...
	function make_std(simulations, aspect)
		features = map(aspect, simulations)
		std_aspect = zeros(Float64, T)
		for t = 1:T
			std_aspect[t] = std([features[s][t] for s = 1:length(simulations)])
		end
		return std_aspect
	end
	...
	p = plot()
	S_mean = make_mean(simulations, (sim) -> sim.S)
	S_std = make_std(simulations, (sim) -> sim.S)
	I_mean = make_mean(simulations, (sim) -> sim.I)
	I_std = make_std(simulations, (sim) -> sim.I)
	R_mean = make_mean(simulations, (sim) -> sim.R)
	R_std = make_std(simulations, (sim) -> sim.R)

	
	plot!(p, S_mean, yerr=S_std, alpha=0.1, markerstrokecolor=:auto)
	plot!(p, I_mean, yerr=I_std, alpha=0.1, markerstrokecolor=:auto)
	plot!(p, R_mean, yerr=R_std, alpha=0.1, markerstrokecolor=:auto)
	return p
end
...
function step!(agents::Vector{Agent}, infection::AbstractInfection)
	...
	interact!(agent1, agent2, infection)
	agents
end
...
let
	...
	simulation_duration = length(simulations[1].I)
	infectious = map((sim) -> sim.I, simulations)
	mean_infectious = zeros(Float64, simulation_duration)
	mean = (x) -> sum(x) / length(x)
	for t = 1:simulation_duration
		mean_infectious[t] = mean([infectious[s][t] for s = 1:length(simulations)])
	end
	println(size(mean_infectious))
	plot!(p, 1:1000, mean_infectious, lw=3)
	p
end
...
# ╔═╡ fe3b70b4-18f2-11eb-3f56-7d4bc42f20f8
sir_mean_error_plot(simulations)

# ╔═╡ 4ad11052-042c-11eb-3643-8b2b3e1269bc
begin
	# 1) Percentage of infected = (I + R) / N
	sim = simulations[12]
	N = sim.S[1] + sim.I[1] + sim.R[1]
	total_infected = (sim.I[end] + sim.R[end]) / N
	
	# 2) Day with the largest infections
	T = length(sim.I)
	largest_infection = maximum([sim.I[i] - sim.I[i-1] for i = 2:T])
	
	# 3) Day with the most infected
	most_infected = maximum(sim.I)
	
	(total_infected, largest_infection, most_infected)
end

# ╔═╡ 1c6aa208-04d1-11eb-0b87-cf429e6ff6d0
sir_mean_plot(repeat_simulations(
	100,1000,
	InfectionRecovery(p_infection, p_recovery),
	20
))
...
reinfection_simulations = repeat_simulations(10000, 1000, Reinfection(0.01, 0.001), 20)
