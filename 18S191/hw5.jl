...
function make_tuple(coord::Coordinate)
...
function collide_boundary(c::Coordinate, L::Number)
	inbounds = (x) -> max(min(x, L), -L)
	return Coordinate(inbounds(c.x), inbounds(c.y))
end
...
function trajectory(c::Coordinate, n::Int, L::Number)
	return accumulate((:+ ∘ collide_boundary), rand(possible_moves, n), init=w)
end
...
let
	...
	n = 1000
	for _ = 1:10
		plot_trajectory!(p, trajectory(Coordinate(0, 0), n))
	end
	p
end
...
# ╔═╡ 17e5f88a-19d3-11eb-252a-a16ffd06ee73
function plot_simulation!(p, sim)
	plot!(p, sim.S, label="Susceptible")
	plot!(p, sim.I, label="Infected")
	plot!(p, sim.R, label="Recovered")
end
...
causes_outbreak = CollisionInfectionRecovery(0.5, 1e-6)

# ╔═╡ 269955e4-0a46-11eb-02cc-1946dc918bfa
does_not_cause_outbreak = CollisionInfectionRecovery(0.5, 0.01)
...
HW4 didn't require the agents to be in contact so every step was more likely to spread infections.
"""
...
# ╔═╡ 1a06c098-19da-11eb-19af-8b02cec308eb
...
function initialize(N::Number, L::Number)
	agents = [Agent(Coordinate(rand(-L:L), rand(-L:L)), S) for _ = 1:N]
...
# ╔═╡ aa95165c-19d3-11eb-101f-d39ae8438d78
function is_susceptible(agent::Agent)
	return agent.status == S
end

# ╔═╡ b32de2ee-19d3-11eb-305f-f75b2976f7cf
...
function visualize(agents::Vector, L)
	p = plot()
	scatter!(p, make_tuple.(position.(agents)), c=color.(agents), ratio=1)
	return p
end
...
function interact_common!(agent::AbstractAgent, source::AbstractAgent, infection::CollisionInfectionRecovery)
	function infectable(agent1, agent2)
		if agent1.status == S && agent2.status == I
			return true
		elseif agent2.status == S && agent.status == I
			return true
		else
			return false
		end
	...
	function infect!(agent1, agent2)
		agent1.status = I
		agent2.status = I
	end
	if position(agent) == position(source) && infectable(agent, source) && rand() < infection.p_infection
		infect!(agent, source)
	end
...
function initialize_social(N, L)
	...
	agents = [
		SocialAgent(
			Coordinate(rand(-L:L), rand(-L:L)),
			S, 0, rand(social_scores)
		) for _ = 1:N
	]
	rand(agents).status = I
	return agents
...
function interact!(agent::SocialAgent, source::SocialAgent, infection::CollisionInfectionRecovery)
	p_risky = social_score(agent) + social_score(source)
	old_agent_status = agent.status
	old_source_status = source.status
	if rand() < p_risky
		interact_common!(agent, source, infection)
	end
	if old_agent_status == S && agent.status == I
		source.num_infected += 1
	end
	
	if old_source_status == S && source.status == I
...
function step!(agents::Vector, L::Number, infection::AbstractInfection)
	source = rand(agents)
	source.position = collide_boundary(position(source) + rand(possible_moves), L)
	for other_agent in filter((a) -> a != source, agents)
		interact!(other_agent, source, infection)
	end
	return agents
end

# ╔═╡ c2cd95d2-19ce-11eb-1914-2931c42c9189
function sweep!(agents, L, infection)
...
# ╔═╡ 9a4ec988-1a90-11eb-15d6-518b5e938ccf
function is_susceptible(agent::AbstractAgent)
	return agent.status == S
end

# ╔═╡ a5c368aa-1a90-11eb-310d-9f43cd54f3cf
function is_infected(agent::AbstractAgent)
	return agent.status == I
end

# ╔═╡ 10a1128a-19d3-11eb-1ca8-25cb4b6880b9
function simulate(N::Integer, L::Integer, T::Integer, infection::AbstractInfection)
	agents = initialize(N, L)

	S_counts = zeros(Integer, T)
	I_counts = zeros(Integer, T)
	R_counts = zeros(Integer, T)
	for t = 1:T
		sweep!(agents, L, infection)
		S_counts[t] = length(filter(is_susceptible, agents))
		I_counts[t] = length(filter(is_infected, agents))
...
# ╔═╡ e0fe150e-19d3-11eb-22e9-e18f0ff8ba81
function simulate_and_plot(N, L, infection)
	simulation = simulate(N, L, k_sweep_max, infection)
	p = plot()
	plot_simulation!(p, simulation)
end

# ╔═╡ d9193342-1a9a-11eb-3a7a-4dce8ff9c959
function simulate_and_plot()
	...
	@gif for t in 1:Tmax
		...
		push!(Ss, count(is_susceptible, social_agents))
		push!(Is, count(is_infected, social_agents))
...
# ╔═╡ dc62374a-19cf-11eb-0a22-5f579f294c48
begin
	N = 50
	L = 30
	simulate_and_plot(N, L, pandemic)
end

# ╔═╡ 92260370-19d2-11eb-1b5e-25c02edc8840
let
	function plot_simulation_alpha!(p, sim)
		plot!(p, sim.S, alpha=0.5, color="red")
		plot!(p, sim.I, alpha=0.5, color="green")
		plot!(p, sim.R, alpha=0.5, color="blue")
	end
	simulations = [simulate(N, L, k_sweep_max, pandemic) for _ = 1:50]
	p = plot()
	for sim in simulations
		plot_simulation_alpha!(p, sim)
	end
	p
end

# ╔═╡ 4d4548fe-0a66-11eb-375a-9313dc6c423d
simulate_and_plot(100, 20, causes_outbreak)

# ╔═╡ 36617c88-19d0-11eb-12b3-6988b8821a4e
simulate_and_plot(100, 20, does_not_cause_outbreak)
...
# ╔═╡ 60fe0d94-1a98-11eb-19ed-a511297177b9
...
begin
	test_agents = Iterators.flatten([simulate_agents() for _ = 1:20])
	
	scatter(social_score.(test_agents), num_infected.(test_agents), alpha=0.5)
end
