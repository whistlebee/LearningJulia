...
A smaller value of B indicates a climate system which is more resistant in terms of change in temperature, as in it will reach the equilibrium quicker. It is called the climate feedback parameter because it represents how the climate will react to an increase of CO2. 
"""
...
When B is great than or equal to zero, it will never reach an equilibrium.
"""
...
histogram(ECS_samples, label=nothing, xlabel="ECS", ylabel="samples")
...
# ╔═╡ c9c362d6-482a-11eb-3ef5-a3d535e49ce7
ECS(B=mean(B_samples))

# ╔═╡ 184d6ed4-48ae-11eb-3f1a-3d3b9a15b158
...
Accounting for uncertainty in feedbacks makes our expectation of global warming worse.
"""
...
simulated_rcp85_model.T[2100 - Integer(simulated_rcp85_model.t[1])]
...
expected_double_CO2_year = let
	findfirst((x) -> x >= 2 * Model.CO2_PI, Model.CO2_RCP85.(t)) + t[1]
end

# ╔═╡ 00985f18-489c-11eb-34b0-23392f908be7
...
function prob_greater_2100_2°(CO2::Function)
	response = map(b -> temperature_response(CO2, b), B_samples)
	preindustrial_temp = temperature_response(t -> 280)
	count(x -> x > preindustrial_temp+2, response) / length(response)
end

# ╔═╡ 292b8c62-48af-11eb-0922-513900ce0cd0
prob_greater_2100_2°(Model.CO2_RCP26)

# ╔═╡ 2f4bf474-48af-11eb-3150-4b12cb571d97
prob_greater_2100_2°(Model.CO2_RCP85)
...
# ╔═╡ e5f27aa8-4964-11eb-1bb0-4330d22f72dd
Model.RCP26
...
@bind log_CO2_slider Slider(log10(CO2min):0.1:log10(CO2max); show_value=true)
...
let
	...
	ebm = Model.EBM(14, 1850, 1, x -> exp10(log_CO2_slider))
	
	step_model!(ebm, exp10(log_CO2_slider))
...
# ╔═╡ 6c7aab18-4965-11eb-39ad-3169491c2bad
function equilibrium_temperature(CO2::Real)
	ebm = Model.EBM(-48.0, 1850, 1, x -> CO2)
	Model.run!(ebm, 3000)
	ebm.T[end]
end

# ╔═╡ d1c97318-4966-11eb-37f0-a70220626418
equilibrium_temperature(280)

# ╔═╡ d9808a24-4966-11eb-36de-3dea265de9f9
equilibrium_temperature(1000)

# ╔═╡ 9eb07a6e-2687-11eb-0de3-7bc6aa0eefb0
co2_to_melt_snowball = let
	logCO2 = log10(CO2min)
	temp = equilibrium_temperature(exp10(logCO2))
	while temp < -10.0
		logCO2 += 0.1
		temp = equilibrium_temperature(exp(logCO2))
	end
	exp10(logCO2)
end
