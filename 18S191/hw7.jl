...
# ╔═╡ 1035a51a-21ab-11eb-08dc-6bc26010f61c
begin
	position(p::Photon) = p.p
	position(w::Wall) = w.position
	direction(p::Photon) = p.l
	normal(w::Wall) = w.normal
end
...
function interact(photon::Photon, hit::Intersection{Wall})
	return Photon(
		hit.point,
		reflect(photon.l, hit.object.normal),
		photon.ior
	)
end
...
# ╔═╡ 3c44cbe4-270b-11eb-09db-45a7396f282b
begin
	position(s::Sphere) = s.center
...
function intersection_distance(photon::Photon, wall::Wall)
	return (
		- ((position(photon) .- position(wall)) ⋅ normal(wall))
		/
		(direction(photon) ⋅ normal(wall))
	)
end
...
function intersection(photon::Photon, wall::Wall; ϵ=1e-3)
	...
	result = if D > ϵ
		p_intersection = position(photon) + D*direction(photon)
		Intersection(wall, D, p_intersection)
	else
		Miss()
	end
	return result
...
function intersection(photon::Photon, sphere::Sphere; ϵ=1e-3)
	a = direction(photon) ⋅ direction(photon)
	b = 2*direction(photon) ⋅ (position(photon) - position(sphere))
	c = (position(photon) - position(sphere)) ⋅ (position(photon) - position(sphere)) - radius(sphere)^2
	d = b^2 - 4*a*c
	nroots = if d < 0
		return Miss()
	elseif d == 0
		root = (-b + √d) / (4*a*c)
		distance = root*norm(direction(photon))
	elseif d > 0
		±(x, y) = (x+y, x-y)
		roots = (-b ± √d) ./ (2*a)
		distances = roots.*norm(direction(photon))
		distance, ind = findmin(distances)
		root = roots[ind]
		# Case when photon starts on the edge of the sphere.
	...
	# Case when the sphere is behind the ray
	if distance < 0
		return Miss()
	end
	new_point = position(photon) + distance*direction(photon)
	return Intersection(sphere, distance, new_point)
end
...
# ╔═╡ dcf944e4-27f5-11eb-31d0-5ff76574c5d0
let
	test_sphere_1 = Sphere(
		[10, -10],
		2,
		1.5,
	)
	p = plot_scene([test_sphere_1, box_scene...])
	test_sphere_intersection_1 = intersection(philip, test_sphere_1)
	plot_photon_arrow!(p, philip, 4; label="Philip")
	if test_sphere_intersection_1 isa Intersection
		scatter!(p, test_sphere_intersection_1.point[1:1], test_sphere_intersection_1.point[2:2], label="Intersection point")
	end
	
	p |> as_svg
end

# ╔═╡ e33493ee-2c81-11eb-3166-a9f9d0991b14
philip

# ╔═╡ d5e0154c-2c81-11eb-1910-d17b7566af9e
let
	test_sphere_1 = Sphere(
		[6, 0],
		3,
...
function interact(photon::Photon, hit::Intersection{Sphere})
	n̂ = sphere_normal_at(position(photon), hit.object)
	n1 = photon.ior
	n2 = hit.object.ior
	...
	return Photon(
		hit.point,
		refract(direction(photon), n̂, n1, n2),
		is_entering ? n2 : n1
	)
end
...
function trace(photon::Photon, scene::Vector{<:Object}, N)
	traced = Array{Photon}(undef, N)
	traced[1] = step_ray(photon, scene)
	for i = 2:N
		traced[i] = step_ray(traced[i-1], scene)
	end
	return traced
end
...
test_lens_photon = Photon([0,0.5], [1,0], 1.0)
