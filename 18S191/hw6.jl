...
function euler_integrate(fprime::Function, fa::Number, 
	...
	h = step(T)
	res = zeros(Float32, length(T))
	res[1] = euler_integrate_step(fprime, fa, first(T), h)
	for i = 2:length(T)
		res[i] = euler_integrate_step(fprime, res[i-1], T[i], h)
	end
...
function euler_SIR(β, γ, sir_0::Vector, T::AbstractRange)
	...
	ret = [sir_0]
	for _ = 1:num_steps-1
		push!(ret, euler_SIR_step(β, γ, ret[end], h))
	end
...
Not rapid. Around 75% of people get infected in total
"""
...
@bind β_vis Slider(0:0.05:1)

# ╔═╡ e8fa7166-1f2e-11eb-3ce7-ab004acaf60b
@bind γ_vis Slider(0:0.05:1)

# ╔═╡ 9958feea-1f36-11eb-3142-ff952b083623
begin
	plot_sir!(
		plot(),
		0:0.1:60.0,
		euler_SIR(β_vis, γ_vis, 
			[0.99, 0.01, 0.00], 
		sir_T)
	)
end
...
function gradient_descent_2d_step(f, x0, y0; η=0.01)
	xy = [x0, y0]
	slopes = [
		finite_difference_slope(a -> f(a, xy[2]), x0),
		finite_difference_slope(b -> f(xy[1], b), y0)
	]
	return xy .- η .* slopes
end
...
function gradient_descent_2d(f, x0, y0; η=0.01)
	...
	diff = Inf
	while diff > 1e-18
		new_xy = gradient_descent_2d_step(f, xy[1], xy[2], η=η)
		diff = sum((xy - new_xy) .^2)
		xy = new_xy
	end
	return xy
end
...
# ╔═╡ 9b5e774c-217f-11eb-0479-d7b5698483cb
md"""
Yes, it depends on the starting location.
"""
...
function loss_dice(μ, σ)
	fx = [gauss(x, μ, σ) for x in dice_x]
	return sum((fx .- dice_y) .^ 2)
end
...
found_μ, found_σ = let
	μ0, σ0 = 30, 1
	gradient_descent_2d(loss_dice, μ0, σ0, η=0.03)
end
...
function loss_sir(β, γ)
	sir_0 = first(hw4_results)
	N = length(hw4_results)
	extrapolated = euler_SIR(β, γ, sir_0, 1:N)
	s = 0.0
	for i = 1:N
		@inbounds s += sum((hw4_results[i] .- extrapolated[i]) .^ 2)
	end
	return s
end

# ╔═╡ ee20199a-12d4-11eb-1c2c-3f571bbb232e
loss_sir(0.018, 0.0021)
...
found_β, found_γ = let
	...
	β0, γ0 = 0.018, 0.0021
	gradient_descent_2d(loss_sir, β0, γ0, η=1e-9)
end
